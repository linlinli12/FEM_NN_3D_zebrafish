...
EdgeSplitter::PreEdgeSplitInfo::PreEdgeSplitInfo( size_t edge, size_t tri0, size_t tri1, size_t vertex_a, 
                                                 size_t vertex_b, 
                                                 size_t vertex_c, 
                                                 size_t vertex_d  ) :
    m_edge(edge), m_tri0(tri0), m_tri1(tri1), m_vertex_a(vertex_a), m_vertex_b(vertex_b), m_vertex_c(vertex_c), m_vertex_d(vertex_d)
{
}
...
EdgeSplitter::PostEdgeSplitInfo::PostEdgeSplitInfo(const PreEdgeSplitInfo& pre_split_info, 
                                                   size_t new_vertex, 
                                                   size_t new_tri0, 
                                                   size_t new_tri1, 
                                                   size_t new_tri2, 
                                                   size_t new_tri3 ) :
    m_pre_split_info(pre_split_info),
    m_new_vertex(new_vertex),
    m_new_tri0(new_tri0),
    m_new_tri1(new_tri1),
    m_new_tri2(new_tri2),
    m_new_tri3(new_tri3)
...
{   
    ...
    // Check with any observers
    
    PreEdgeSplitInfo pre_info( edge, tri0, tri1, vertex_a, vertex_b, vertex_c, vertex_d );

    for ( size_t i = 0; i < m_observers.size(); ++i )
    {
        if ( !m_observers[i]->operationOK( m_surf, pre_info ) )
        {
            return false;
        }
    }
    ...
    // Do the actual splitting, interpolate concentrations
    
    double new_vertex_mass = 0.5 * ( m_surf.m_masses[ vertex_a ] + m_surf.m_masses[ vertex_b ] );

    // double new_vertex_cB = 0.5 * ( m_surf.m_cB[ vertex_a ] + m_surf.m_cB[ vertex_b ] );
    // double new_vertex_cC = 0.5 * ( m_surf.m_cC[ vertex_a ] + m_surf.m_cC[ vertex_b ] );
    // double new_vertex_cBC = 0.5 * ( m_surf.m_cBC[ vertex_a ] + m_surf.m_cBC[ vertex_b ] );
    // double new_vertex_cN = 0.5 * ( m_surf.m_cN[ vertex_a ] + m_surf.m_cN[ vertex_b ] );
    // double new_vertex_cBN = 0.5 * ( m_surf.m_cBN[ vertex_a ] + m_surf.m_cBN[ vertex_b ] );
    // double new_vertex_cS = 0.5 * ( m_surf.m_cS[ vertex_a ] + m_surf.m_cS[ vertex_b ] );
    // double new_vertex_cTld = 0.5 * ( m_surf.m_cTld[ vertex_a ] + m_surf.m_cTld[ vertex_b ] );
    double species_n =  m_surf.m_cspecies[0].size();
    //std::cout<<"going to assign new concentrations, thete are "<<species_n<<" species\n";
    std::vector<double> new_vertex_cspecies(species_n);
    for ( size_t i = 0; i < m_surf.m_cspecies[0].size(); ++i ){
    new_vertex_cspecies[i] = 0.5 * ( m_surf.m_cspecies[ vertex_a ][i] + m_surf.m_cspecies[ vertex_b ][i] );
    }
    //std::cout<<"assigned new concentrations\n";
    // size_t vertex_e = m_surf.add_vertex( new_vertex_smooth_position, new_vertex_mass );
    //size_t vertex_e = m_surf.add_vertex( new_vertex_smooth_position, new_vertex_mass, new_vertex_cB,new_vertex_cC,new_vertex_cBC ,new_vertex_cN,new_vertex_cBN,new_vertex_cS,new_vertex_cTld);
    size_t vertex_e = m_surf.add_vertex( new_vertex_smooth_position, new_vertex_mass,new_vertex_cspecies);
    //std::cout<<"added the vertex\n";
    ...
    size_t new_tri0 = m_surf.add_triangle( Vec3st( vertex_a, vertex_e, vertex_c ) );
    size_t new_tri1 = m_surf.add_triangle( Vec3st( vertex_c, vertex_e, vertex_b ) );
    size_t new_tri2 = m_surf.add_triangle( Vec3st( vertex_d, vertex_b, vertex_e ) );
    size_t new_tri3 = m_surf.add_triangle( Vec3st( vertex_d, vertex_e, vertex_a ) );      
    size_t new_vertex = vertex_e;

    PostEdgeSplitInfo post_info( pre_info, new_vertex, new_tri0, new_tri1, new_tri2, new_tri3 );
...
void EdgeSplitter::process_mesh()
{
    ...
    while ( split_occurred )
    {
        ...
        {
            size_t longest_edge = iter->edge_index;
        ...
        split_occurred |= large_angle_split_occurred;

    }   // while split_occurred
    
}

void EdgeSplitter::add_observer( EdgeSplitObserver* observer )
{
