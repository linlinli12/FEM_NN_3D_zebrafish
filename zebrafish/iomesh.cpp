...
void read_ecomsol(NonDestructiveTriMesh &mesh, std::vector<Vec3d> &x, std::vector<std::vector<double> > &c_species,const char* filename)
{
    // read nodes
    x.clear();
    std::ifstream myfile(filename);
    ...
    if (myfile.is_open())
    {
        ...
        while ( getline (myfile,line) )
        {
            ...
            if (found!=std::string::npos)
            {
                ...
                ss0>>species_n;
                getline (myfile,line);
                std::stringstream ss01(line);
                ss01>>n_conc;
                // found the beginning of the nodal concentrentrations, read
                for(int i =0;i<n_conc;i++){
                    getline (myfile,line);
                    std::stringstream ss1(line);
                    std::vector<double> in_species; 
                    in_species.clear();
                    for (int j = 0; j<species_n;j++){
                        // double cBi,cCi,cBCi,cNi,cBNi,cSi,cTldi;
                        // ss1>>cBi;
                        // ss1>>cCi;
                        // ss1>>cBCi;
                        // ss1>>cNi;
                        ...
                        // c_BN.push_back(cBNi);
                        // c_S.push_back(cSi);
                        // c_Tld.push_back(cTldi);
                        double init_conscentration;
                        ss1>>init_conscentration;
                        in_species.push_back(init_conscentration);
                        }
                    c_species.push_back(in_species);
                }
            }
        }
    }
...
void write_vtk(const NonDestructiveTriMesh &mesh, const std::vector<Vec3d> &x, const std::vector<double> &c_1,const std::vector<double> &c_2,const std::vector<double> &c_3,const std::vector<double> &c_4,double current_time, const char* filename)
{
    std::ofstream savefile(filename);
    if (!savefile) {
        throw std::runtime_error("Unable to open output file.");
    ...
    savefile<<"POINT_DATA "<<c_1.size()<<"\n";
	savefile<<"SCALARS concentrations float 4\n";
	savefile<<"LOOKUP_TABLE default\n";
	for(int i=0;i<c_1.size();i++)
    {
        savefile<<c_1[i]<<" "<<c_2[i]<<" "<<c_3[i]<<" "<<c_4[i]<<"\n";
    }
    
    savefile.close();
}

// for write 3 speices results
void write_vtk(const NonDestructiveTriMesh &mesh, const std::vector<Vec3d> &x, const std::vector<double> &c_1,const std::vector<double> &c_2,const std::vector<double> &c_3,double current_time, const char* filename)
{
    std::ofstream savefile(filename);
    if (!savefile) {
        throw std::runtime_error("Unable to open output file.");
